...
module.exports = (plugin) => {
  ...
  plugin.controllers.user.logout = async (ctx) => {
    try {
      ...
      if (!token) {
        try {
          await strapi.db.query("api::block-list.block-list").create({
            data: { token: ctx.request.header.authorization.split(" ")[1] },
          });
          ctx.body = { message: "Logout Success" };
        } catch (error) {
          console.log("Fail");
          ctx.body = { message: "Something wrong!!" };
        }
      } else {
